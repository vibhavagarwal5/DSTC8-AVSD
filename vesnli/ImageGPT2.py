...
class ImageGPT2Model(GPT2Model):
    def __init__(self, config):
        super(ImageGPT2Model, self).__init__(config)
    ...
    def forward(
        ...
        if position_ids is None:
            position_ids = torch.arange(
                past_length, input_embs.size(-2) + past_length, dtype=torch.long, device=input_embs.device)
            position_ids = position_ids.unsqueeze(
                0).expand_as(input_embs[:, :, 0])
...
class ImageGPT2LMHeadModel(GPT2PreTrainedModel):
    def __init__(self, config):
    ...
    def forward(
        ...
        if labels is not None:
            ...
            loss = loss_text_fct(shift_logits.view(-1, shift_logits.size(-1)),
                                 shift_labels.view(-1))
