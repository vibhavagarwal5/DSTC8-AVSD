...
def get_dataset(tokenizer, data_file, feature_path=None, undisclosed_only=False, n_history=3):
    ...
    for dialog in dialog_data['dialogs']:
        ...
        questions = [tokenize(d['question'], tokenizer)
        ...
        for n in it:
            ...
            if n_history == 0:
                item = {'vid': vid, 'history': [
                    question], 'answer': answer, 'caption': caption}
            else:
                item = {'vid': vid, 'history': history,
                        'answer': answer, 'caption': caption}
...
def build_input_from_segments(caption, history, reply, tokenizer, with_eos=True, video=False, drop_caption=False, train=True):
    ...
    if not drop_caption:
        instance = {}
        sequence = [[bos] + list(chain(*caption))] + \
            history + [reply + ([eos] if with_eos else [])]
        sequence = [[cap] + sequence[0] + [eos]] + [[speaker2 if (
            len(sequence) - i) % 2 else speaker1] + s for i, s in enumerate(sequence[1:])]
        ...
        sequence = [[speaker2 if (len(sequence) - i) %
                     2 else speaker1] + s for i, s in enumerate(sequence)]
